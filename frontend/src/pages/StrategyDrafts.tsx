...
import type { StrategyDraft } from '@/pages/CreateStrategyWizard'
...
interface StrategyDraftsProps {
  drafts: StrategyDraft[]
  onContinue: (draftId: string) => void
  onDelete: (draftId: string) => void
}

export default function StrategyDrafts({ drafts, onContinue, onDelete }: StrategyDraftsProps) {
  if (drafts.length === 0) {
    return (
      <div className="flex h-full flex-col">
        <div className="px-6 py-5 space-y-3">
          <Breadcrumbs items={[{ label: 'Strategy' }, { label: 'Drafts' }]} />
          <h1 className="text-lg font-semibold text-foreground">Drafts</h1>
        </div>
        <BuyerEmptyState variant="no-drafts" />
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
      ...
      <div className="px-6 pb-6 space-y-3">
        {drafts.map((draft) => {
          const name = draft.formState.name || 'Untitled Strategy'
            ...
            >
                  ...
                  {WIZARD_STEPS.map((_, i) => (
                    <div
                      ...
                      className={`h-1.5 w-6 rounded-full ${
                        i < stepsCompleted ? 'bg-mode-strategy' : 'bg-muted'
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Button
                  ...
                  className="bg-mode-strategy hover:bg-mode-strategy/80 text-white"
                  size="sm"
                >
                  Continue
                  <ArrowRight size={14} />
